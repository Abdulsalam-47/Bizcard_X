    ...
    if txts:
        ...
        for i in range(2, len(txts)):
            if txts[i].startswith("+") or (txts[i].replace("-", "").isdigit() and '-' in txts[i]):
                ...
                extd_dict["WEBSITE"].append(txts[i].lower())
            elif "Tamil Nadu" in txts[i] or "TamilNadu" in txts[i] or txts[i].isdigit():
                extd_dict["PINCODE"].append(txts[i])
            elif re.match(r'^[A-Za-z]', txts[i]):
                extd_dict["COMPANY NAME"].append(txts[i])
            else:
                extd_dict["ADDRESS"].append(re.sub(r'[,;]', '', txts[i]))
...
st.markdown('<hr style="border-top: 3px solid; background-image: linear-gradient(to right, violet, indigo, blue, green, yellow, orange, red);">', unsafe_allow_html=True)

# Additional markdown with creator's name in a rainbow gradient
st.markdown("""
    <h1>This OCR app is created by <span style="background: linear-gradient(to right, violet, indigo, blue, green, yellow, orange, red); -webkit-background-clip: text; color: transparent;">ABDUL SALAM</span>!</h1>
...
if select == "Home":
    ...
    if st.button("Delete"):
        if st.checkbox(f"Are you sure you want to delete {selected_name}? This action cannot be undone."):
            cursor.execute(f"DELETE FROM bizcard_info WHERE NAME = '{selected_name}'")
            mydb.commit()
